...
export class HomePage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.releaseLinks = page.locator('#sidebar a[href="/projects/redmine/wiki/Download"]');
    }
    ...
    async clickFirstReleaseLink(): Promise<void> {
        await this.releaseLinks.first().click();
    }
    ...
    async verifyPageOpened(): Promise<void> {
        ...
        await expect(this.pageTitle).toHaveText('Redmine');
    }
    ...
    async verifyAtLeastOneReleaseIsDisplayed(): Promise<void> {
        const releaseCount = await this.releaseLinks.count();
        expect(releaseCount).toBeGreaterThan(0);
        await expect(this.releaseLinks.first()).toBeVisible();
    }
}